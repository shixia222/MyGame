...
public class Body extends Frame implements ActionListener {
	//游戏宽度
	...
	//创建我的坦克
	Role myTank = new Role(220, 560, true,Role.Direction.STOP, this);
	//创建鹰碉堡
	...
	public void paint(Graphics g) {
		...
		if(roles.size()<=0){
			for(int i=0; i<6; i++) {
				if(i<3){
				    roles.add(new Role(50 + 40*(i+1), 50, false,Role.Direction.D, this));
				}else{
					roles.add(new Role(50 + 40*(i+1), 50, false,Role.Direction.R, this));
				}
			}
		}
		...
		//g.drawString("坦克数量:" + roles.size(), 10, 110);
		...
		for(int i=0; i<roles.size(); i++) {
			...
			myTank.collidesWithTanks(roles);
			t.collidesWithTanks(roles);
	...
	public void newTank(){
		for (int i = 0; i < 10; i++) {
			if (i < 5) {
				roles.add(new Role(50 + 40 * (i + 1), 55, false,
						Role.Direction.D, this));
			} else {
				roles.add(new Role(50 + 40 * (i + 1), 55, false,
						Role.Direction.R, this));
			}
		}
	}
	/**
